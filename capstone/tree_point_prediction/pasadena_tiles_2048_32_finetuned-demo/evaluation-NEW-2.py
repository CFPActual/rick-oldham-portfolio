...
if len(gdf_predictions) == 0:
    raise ValueError("No predictions found in merged_pasadena.json")
...
# Find recall at 12 m (or closest threshold) # after computing 'thresholds' and 'recalls'
# recall_at_12 = float(recall_df.loc[recall_df['threshold'] == 12, 'recall'])
# plt.axvline(x=12, color='red', linestyle='--', label=f'12m Threshold ({recall_at_12:.2%})')
# after computing thresholds, recalls
t12 = 12
i12 = np.where(thresholds==t12)[0][0]
plt.axvline(x=t12, color='red', linestyle='--', label=f'{t12}m Threshold ({recalls[i12]*100:.2f}%)')
